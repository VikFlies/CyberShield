...
const QuizModule = () => {
  ...
  const [categories, setCategories] = useState<Record<string, { correct: number; total: number }>>({});
  const timerRef = useRef<number | null>(null);

  // Sélection aléatoire de 10 questions sans répétition via filter + sort
  const startQuiz = () => {
  ...
  const handleAnswer = useCallback((idx: number) => {
    ...
    if (isCorrect) {
      ...
      if (newStreak >= 3) points = Math.floor(points * 1.5); // streak bonus
  ...
  // Recommandations basées sur catégories échouées
  const getRecommendations = () => {
    ...
    if (recs.length === 0) recs.push('Excellent niveau ! Continuez à vous former.');
    return recs;
  };
      ...
      <div className="space-y-6 max-w-lg">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Testez vos connaissances en cybersécurité avec 10 questions aléatoires.
          </p>
        </div>
        ...
        <div className="card-cyber flex gap-3">
          ...
          <div className="text-sm text-muted-foreground">
            ...
            <p>• <strong className="text-foreground">Bonus rapidité</strong> : répondez vite pour plus de points</p>
            <p>• <strong className="text-foreground">Streak</strong> : 3 bonnes réponses consécutives = +50% de points</p>
          </div>
        </div>
  ...
  if (phase === 'result') {
    const st = getState();
    const rank = st.quizTopScores.findIndex(r => r.score === score && r.date === new Date().toLocaleDateString('fr-FR'));

    return (
      <div className="space-y-6 max-w-lg">
        <div className="text-center">
          <Trophy className="w-16 h-16 text-warning mx-auto mb-4" />
          <h2 className="text-2xl font-bold text-foreground">Quiz terminé !</h2>
        </div>

        <div className="card-cyber text-center">
          <p className="text-4xl font-bold text-primary font-mono">{score}</p>
          <p className="text-muted-foreground text-sm mt-1">points</p>
        </div>

        {/* Catégories */}
        <div className="card-cyber space-y-3">
          <h3 className="text-sm font-semibold text-foreground">Détail par catégorie</h3>
